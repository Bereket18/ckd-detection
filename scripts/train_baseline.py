...
def format_threshold_sweep(sweep, deployed=0.5) -> str:
    """
    The threshold sweep as a table, with the deployed operating point marked.

    The header note is not decoration. These rows are measured on the held-out
    test set, so choosing an operating point by reading down the recall column
    would be selecting a parameter using test data -- and every metric reported
    afterwards would be optimistic. That is the same class of error as the
    leakage in AUDIT.md (P0-3), reached from a different direction. The deployed
    threshold stays 0.5 (model.predict), and config.RISK_BAND_BOUNDS is a
    judgement call, not a fit. Choosing a threshold properly would require a
    validation split carved out of the training data.

    n_fn is shown beside the rates because it is the number that matters
    clinically: on 80 test rows, "recall 0.98" is one missed patient.
    """
    lines = [
        "Threshold sweep (REPORTED, NOT USED FOR SELECTION -- measured on the test",
        "set, so picking a threshold from it would select on test data; deployed",
        f"threshold stays {deployed} i.e. model.predict):",
        "",
        "    thresh   recall   specif   precis     acc   n_FN   n_FP",
    ]
    for row in sweep:
        mark = "  <- deployed" if row["threshold"] == deployed else ""
        lines.append(
            f"    {row['threshold']:6.2f}   {row['recall']:6.4f}   "
            f"{row['specificity']:6.4f}   {row['precision']:6.4f}  "
            f"{row['accuracy']:6.4f}   {row['n_fn']:4d}   {row['n_fp']:4d}{mark}"
        )
    return "\n".join(lines)
...
def main(argv=None):
    ...
    # Brier prints without an interval: it is a mean squared error, not a
    # proportion, so wilson_interval() does not apply to it.
    print(f"  {'brier':12s} {results['brier_score']:.4f}   "
          f"(lower is better; 0.2500 = predicting 0.5 for everyone)")
    print(f"\n  confusion matrix [[TN, FP], [FN, TP]]: {results['confusion_matrix']}")
    print(f"\n{format_threshold_sweep(results['threshold_sweep'])}")
    ...
    tabular_model.append_metrics_history(
        {
            ...
            **{k: results[k] for k in
            ...
            # Logged so "were this run's probabilities better?" is answerable
            # across runs. The threshold sweep is deliberately NOT logged here:
            # nine rows per run would make each line unreadable, and the sweep is
            # already saved in full beside the model it belongs to.
