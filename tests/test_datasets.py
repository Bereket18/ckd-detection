...
# ---------------------------------------------------------------------------
# Loading unlabelled data for scoring (require_target=False)
#
# scripts/predict.py needs a foreign CSV renamed and re-encoded through its
# DatasetSpec, but scoring data legitimately has no labels. The relaxation must
# be exactly that -- and no wider.
# ---------------------------------------------------------------------------

@pytest.fixture
def unlabelled_csv(awkward_csv, tmp_path):
    """The awkward fixture with its label column removed -- i.e. what a clinic
    would actually send for scoring."""
    frame = pd.read_csv(awkward_csv).drop(columns=["Diagnosis"])
    path = tmp_path / "unlabelled.csv"
    frame.to_csv(path, index=False)
    return path


def test_a_label_less_csv_loads_when_the_target_is_not_required(unlabelled_csv):
    """The scoring path: same renaming, same coverage report, no label."""
    spec = DatasetSpec(
        name="unlabelled", filename=str(unlabelled_csv), column_map=COLUMN_MAP
    )
    df, coverage = load_dataset(spec, require_target=False)

    assert config.TARGET_COLUMN not in df.columns
    assert coverage.n_rows == 80
    assert coverage.numeric_present == FIXTURE_NUMERIC
    assert coverage.binary_present == FIXTURE_BINARY
    # The column_map and the numeric coercion still ran -- the relaxation is
    # about the label only.
    assert not set(COLUMN_MAP).intersection(df.columns)
    assert pd.api.types.is_numeric_dtype(df["sc"])


def test_the_default_still_refuses_a_label_less_csv(unlabelled_csv):
    """
    Training on unlabelled data is not a thing, and combine_datasets keeps the
    default -- so a missing label stays an error everywhere it matters.
    """
    spec = DatasetSpec(
        name="unlabelled", filename=str(unlabelled_csv), column_map=COLUMN_MAP
    )
    with pytest.raises(SchemaMismatchError, match="no 'class' column"):
        load_dataset(spec)


def test_not_requiring_a_target_does_not_weaken_a_target_that_is_present(tmp_path):
    """
    The narrowness of the relaxation, which is the part worth a test: a file that
    HAS labels and cannot read them is a broken spec whether or not the caller
    intends to train. Accepting it here would let predict.py evaluate against
    silently-NaN labels.
    """
    ...
    spec = DatasetSpec(
        name="bad_label_scoring", filename=str(path),
        column_map={"Diagnosis": "class"}, target_map={"Healthy": "notckd"},
    )
    with pytest.raises(SchemaMismatchError, match="target column"):
        load_dataset(spec, require_target=False)


def test_a_value_map_still_applies_when_the_target_is_absent(tmp_path):
    """
    value_map is the other half of what makes scoring a foreign CSV possible, so
    it must run on the unlabelled path too -- and still refuse a value it does
    not cover rather than letting it become an imputed observation.
    """
    path = tmp_path / "scoring_binaries.csv"
    pd.DataFrame({"Hypertension": ["1", "0"], "Age": [50, 60]}).to_csv(path, index=False)
    spec = DatasetSpec(
        name="scoring_binaries", filename=str(path),
        column_map={"Hypertension": "htn", "Age": "age"},
        value_map={"htn": {"1": "yes", "0": "no"}},
    )
    df, _ = load_dataset(spec, require_target=False)
    assert list(df["htn"]) == ["yes", "no"]

    bad_path = tmp_path / "scoring_bad.csv"
    pd.DataFrame({"Hypertension": ["MAYBE"], "Age": [50]}).to_csv(bad_path, index=False)
    with pytest.raises(ValueError, match="value_map does not cover"):
        load_dataset(spec, path=bad_path, require_target=False)
