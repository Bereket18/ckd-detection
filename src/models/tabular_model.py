...
def _as_binary(y):
    """
    Accept either the string labels or an already-numeric 0/1 array.

    _to_binary_target compares against "ckd", so handing it an int array would
    silently return all zeros rather than raising -- and a metric computed
    against all-zeros looks like a real number. Branching on dtype keeps callers
    that pass y_test (strings) and tests that pass [0, 1, 1] both correct.
    """
    array = np.asarray(y)
    if np.issubdtype(array.dtype, np.number):
        return array.astype(int)
    return _to_binary_target(array)
...
def risk_band(probability: float) -> str:
    """
    Qualify a predicted P(CKD) as "LOW", "MODERATE" or "HIGH".

    Bounds come from config.RISK_BAND_BOUNDS so the agent and scripts/predict.py
    cannot drift apart -- the same one-definition rule that AUDIT.md P1-8 was
    filed for.

    MODERATE means "near the decision boundary", which is the case worth naming:
    the binary verdict is produced by model.predict() at 0.5, so a probability of
    0.52 and one of 0.99 currently look identical to a patient although the first
    would have flipped on a couple of trees voting differently. Both sides of the
    boundary qualify -- a notckd verdict at P(CKD)=0.42 is MODERATE too.

    Boundaries are inclusive at the ends (<= low is LOW, >= high is HIGH) so
...
def brier_score(y_true, y_proba) -> float:
    """
    Mean squared error of the predicted probabilities: mean((p - y)^2). Lower is
    better; 0.25 is what predicting 0.5 for everyone scores.

    This is here because the agent now shows the patient a probability, and a
    random forest's predict_proba is the fraction of trees voting for the class
    -- NOT a calibrated probability. "0.80" does not mean 80% of such patients
    have CKD. The honest options were to calibrate the model or to measure and
    disclose; calibrating would change the saved model and could move the
    measured recall below config.MIN_ACCEPTABLE_RECALL, so this project measures
    and discloses (see MODEL_CARD.md).

    Brier rather than a calibration curve because the test set is 80 rows: a
    10-bin reliability diagram would have ~8 rows per bin and be almost entirely
    noise. Brier is a single number that stays meaningful at this sample size.
    Note it conflates calibration and discrimination -- a low score is not proof
    of calibration on its own, which is why the caveat stays regardless.
    """
    y = _as_binary(y_true)
...
def threshold_sweep(model, X_test, y_test, thresholds=DEFAULT_THRESHOLDS) -> list:
    """
    Recall/specificity/precision/accuracy at each decision threshold, as a list
    of dicts (one per threshold, in the order given).

    tune_model optimizes recall, and P2-6 added specificity so that choice could
    be seen rather than assumed. This is the next question a reader asks: what
    would a different threshold have cost? Answering it with a measured table is
    better than answering it with an argument.

    REPORTED, NOT USED FOR SELECTION. These rows are computed on the held-out
    test set, so picking an operating point from them would choose a threshold
    using test data and every metric afterwards would be optimistic -- the same
    class of error as the leakage in AUDIT.md P0-3, arrived at from a different
    direction. The deployed threshold stays 0.5 (i.e. model.predict), and
    config.RISK_BAND_BOUNDS is a judgement call, not a fit. Selecting a threshold
    properly would need a validation split carved out of training data.

    n_fn is listed alongside the rates because it is the number that matters
    clinically: at 80 test rows, "recall 0.98" is one missed patient.
    """
    ...
    for threshold in thresholds:
        ...
        # labels=[0, 1] guarantees a 2x2 matrix even at an extreme threshold
        # where one class is never predicted, which would otherwise unpack as a
        # 1x1 and raise here rather than at the threshold that caused it.
        (tn, fp), (fn, tp) = confusion_matrix(y_bin, y_pred, labels=[0, 1])
        sweep.append({
            "threshold": float(threshold),
            "recall": float(tp / (tp + fn)) if (tp + fn) else 0.0,
            "specificity": float(tn / (tn + fp)) if (tn + fp) else 0.0,
            "precision": float(tp / (tp + fp)) if (tp + fp) else 0.0,
            "accuracy": float((tp + tn) / len(y_bin)) if len(y_bin) else 0.0,
            "n_fn": int(fn),
            "n_fp": int(fp),
        })
    return sweep
...
def evaluate(model, X_test, y_test) -> dict:
    ...
    brier_score and threshold_sweep are included because the agent now shows the
    patient a probability rather than only a label. The first says how good the
    probabilities are; the second says what a different decision threshold would
    have cost. Neither changes the model: the reported point estimates above are
    still the 0.5-threshold predictions the agent actually makes.
    """
    y_bin = _to_binary_target(y_test)
    ...
    return {
        ...
        "brier_score": brier_score(y_bin, y_proba),
        "confusion_matrix": confusion,
        "intervals": _intervals_from_confusion(confusion),
        "threshold_sweep": threshold_sweep(model, X_test, y_test),
