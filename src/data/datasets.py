...
def load_dataset(name_or_spec, path=None, require_target: bool = True):
    """
    ...
    require_target=False allows a file with no label column, for scoring rather
    than training (scripts/predict.py). It exists so a foreign CSV can be scored
    through its DatasetSpec's column_map and value_map -- which is the whole
    point of having those maps; without it, batch scoring would need its own
    renaming code and the mapping would be defined twice. It does NOT relax the
    label checks when a label IS present: unmapped labels still raise, because a
    file that has labels and cannot read them is a broken spec either way.
    Training paths (combine_datasets) keep the default, so a missing label can
    never be silently tolerated where it matters.
    """
    ...
    if not has_target and require_target:
