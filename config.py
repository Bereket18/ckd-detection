...
# The model card, GENERATED by scripts/make_model_card.py rather than written by
# hand. Every figure in it is read from TABULAR_METRICS_PATH. AUDIT.md P1-1
# records what hand-copied metrics do: the baseline accuracy was pasted into
# three files and became false the moment the pipeline changed. A model card is
# the one document where a stale number is least defensible, so it is not a file
# anyone edits.
...
# Band boundaries on P(CKD) -- the model's predicted probability for the CKD
# class -- used to qualify a prediction as LOW / MODERATE / HIGH.
#
# Defined on P(CKD) rather than on "confidence in the shown verdict" so the
# number displayed and the verdict can never appear to disagree. The verdict
# itself still comes from model.predict(), i.e. the 0.5 boundary, so the measured
# accuracy and recall keep describing exactly what a user is shown; the band is
# additional information, not a third outcome.
#
# MODERATE therefore means "near the decision boundary", on either side: a
# LOWER RISK verdict at P(CKD)=0.42 is correctly MODERATE, because two trees
# voting differently would have flipped it. The bounds are deliberately wide.
# They are a judgement about when to tell a patient not to rely on the result
# without a lab test, not something estimated from data -- and they are NOT
# derived from the threshold sweep, which is measured on the test set (see
# tabular_model.threshold_sweep).
RISK_BAND_BOUNDS = (0.35, 0.65)
